...
The application avoids hardcoding movie data on the booking page. Instead, it uses **URL Query Parameters**. When a user clicks "Book Now" on `home.html`, it passes data like this:
...
- **State Management**: When a seat is clicked, the `toggleSeat` function adds/removes the seat ID from a `selectedSeats` array and calculates the total price: `SelectedSeats.length * TicketPrice`.
...
- **Route Guard**: A script at the top of `home.html` and `booking.html` checks for the user in `localStorage`. If missing, it redirects them to `index.html`.

### D. PWA (Progressive Web App) Implementation
- **Manifest**: Defines how the app appears when installed (icons, theme color, start URL).
- **Service Worker (`sw.js`)**: Intercepts network requests. It caches essential files (`index.html`, `styles.css`) so the app loads instantly on mobile even with a slow connection.
